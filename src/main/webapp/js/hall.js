...
function deleteEl(elementMustDelete, arrayWhereSearchElement) {
    let index = arrayWhereSearchElement.indexOf(elementMustDelete);
...
function initHall() {
    ...
    $.ajax({
        ...
        let currentRow = 1;
        let currentSeat = 0;
        $(data).each(function (index, hall) {
            if (index === 0) {
                currentSeat = hall.seat;
                if (hall.status) {
                    $('.heading_screen').after("<ul class='row_1 ul_" + currentRow + "'>"
                        + "<li>" + currentRow + "</li>"
                        + "<li class='hall_seats'>"
                        + "<ul>"
                        + "<li id='" + hall.id + "' class='place free' value='" + currentRow + "'>" + currentSeat + "</li>");
                } else {
                    $('.heading_screen').after("<ul class='row_1 ul_" + currentRow + "'>"
                        + "<li>" + currentRow + "</li>"
                        + "<li class='hall_seats'>"
                        + "<ul>"
                        + "<li id='" + hall.id + "' class='place_taken'>" + currentSeat + "</li>");
                }
            }
            if (data[index].row > currentRow) {
                currentSeat = hall.seat;
                $('.ul_' + currentRow + ' .hall_seats').after("<li>" + currentRow + "</li>");
                currentRow = hall.row;
                if (hall.status) {
                    $('.row_1:last').after("<ul class='row_1 ul_" + currentRow + "'>"
                        + "<li>" + currentRow + "</li>"
                        + "<li class='hall_seats'>"
                        + "<ul>"
                        + "<li id='" + hall.id + "' class='place free' value='" + currentRow + "'>" + currentSeat + "</li>");
            ...
            } else if (data[index].seat > currentSeat) {
                currentSeat = data[index].seat;
                if (index === data.length - 1) {
                    if (hall.status) {
                        $('.hall_seats li:last').after("<li id='" + hall.id + "' class='place free' value='" + currentRow + "'>" + currentSeat + "</li>");
                    } else {
                        $('.hall_seats li:last').after("<li id='" + hall.id + "' class='place_taken'>" + currentSeat + "</li>");
                    }
                    $('.ul_' + currentRow + ' .hall_seats').after("<li>" + currentRow + "</li>");
                } else {
                    if (hall.status) {
            ...
            $('.hall_seats').off().on('click', '.free', function () {
                let id = $(this).attr('id');
                let row = $(this).val();
                let seat = $(this).text();
                storageReservationSeats.push({id: id, row: row, seat: seat});
                $(this).removeClass('free');
                $(this).addClass('active');
            }).on('click', '.active', function (event) {
                deleteEl($(this).attr('id'), storageReservationSeats);
    ...
    $('#pay').off().click(function () {
        if (storageReservationSeats.length > 0) {
            showBookingModal(storageReservationSeats);
        } else {
    ...
    $('#close-booking').off().click(function () {
        ...
        if (name !== '' && name.length >= 5) {
            switcherName = true;
            addSuccessSpan('.name-label', 'Ф.И.О:')
        } else {
            switcherName = false;
        ...
        if (phone !== '' && phone.length >= 10) {
            ...
            switcherPhone = false;
            addErrorSpan('.phone-label', 'Введите корректный номер телефона')
        }

        if (switcherName && switcherPhone) {
            let userData = {
                name: name,
                phone: phone
            };
            setDataToServer(storageReservationSeats, userData);
        }

    })
}

function showBookingModal(storageReservationSeats) {
    let resultString = "";
    let total = $('#price').val() * storageReservationSeats.length;
    $(storageReservationSeats).each(function (index, place) {
        resultString += "ряд " + place.row + " место " + place.seat + "<br>";
    });
    $('#booking').modal('show');
    $('.booking-content .modal-body').remove();
    $('.booking-header').after("<div class='modal-body booking-body'></div>");
    $('.booking-body').append(
        "<form>"
        + "<div class='form-group has-feedback name-label'>"
        + "<label for='name'>Ф.И.О: </label>"
        + "<input name='name' style='background-color: #F5F3F0' class='form-control' id='name' placeholder='Ф.И.О'>"
        + "</div>"
        + "<div class='form-group has-feedback phone-label'>"
        + "<label for='phone'>Номер телефона: </label>"
        + "<input type='text' name='phone' style='background-color: #F5F3F0' class='form-control' id='phone' placeholder='Номер телефона'>"
        + "</div>"
        + "</form>"
        + "<div><label>Вы выбрали:</label><br>" + resultString + "<br><label>Сумма к оплате " + total + "р</label></div>");
    $(".booking-body #phone").inputmask("+7(999)999-99-99")
}


function setDataToServer(storage, user) {
    ...
    $.ajax({
        ...
        $('#booking').modal('hide');
        alert("Данные успешно переданны, приятного просмотра!");
        initHall();
    }).fail(function (err) {
        console.log(err);
    });
